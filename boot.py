    ...
    if code:
        ...
        for i in code:
           ...
           if i.classid()==1: # bicycle
            ...
            #a=img.lens_corr(strenght=1.2, zoom=1.0)
            a = img.draw_string(i.x(),i.y(), classes[i.classid()], color=(255,0,0), scale=3)
            ...
            print(code)
            #a=img.draw_rectangle(i.rect(),color = (255, 255, 0),thickness=4)
            #a=img.find_lines(x_stride=2, threshold=1000, theta_margin=25, rho_margin=25)
            a=img.draw_string(i.x(),i.y(), "No objects found", color=(255,0,0), scale=1)
